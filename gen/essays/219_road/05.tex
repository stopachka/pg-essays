...
{[}3{]} Today a lot of the top hackers are using languages far removed
...
worst consequences. {[}5{]} The traditional ``break everything and then
...
vaguer, buglike things, like features that confused users. {[}6{]} They
...
afford. {[}8{]} Software is particularly suitable for price
...
deliberately turn a blind eye to some kinds of piracy. {[}9{]} With
...
in line. {[}10{]} At Viaweb we had external forces in plenty to keep us
...
things no mainframe software could do. {[}11{]} VisiCalc was such an
...
for it. {[}13{]} You see this less with Windows, because hackers don't
...
startup competing with a big company, it's good news. {[}15{]} Web-based
...
you won't have to tweak it for every new client. {[}16{]}
...
dropping Java in the most recent version of Explorer.
...
{[}7{]} Robert Morris wrote the ordering system, which shoppers used to
...
{[}8{]} Price discrimination is so pervasive (how often have you heard a
...
{[}9{]} In \emph{No Logo,} Naomi Klein says that clothing brands favored
by ``urban youth'' do not try too hard to prevent shoplifting because in
their target market the shoplifters are also the fashion leaders.
...
{[}14{]} One thing that would help Web-based applications, and help keep
